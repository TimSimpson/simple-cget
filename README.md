...
### Installing a simple dependent executable ###
...
* Build the project itself and optionally install it using `cget build` or `cget install`. Both of these commands will automatically initialize a cget-env and install dependencies listed in a requirements.txt file, so this could be collapsed into one step.

To show this, enter the zdapp directory and run `cget build` now:

    cd zdapp
    cget build

This will create a new `cget` directory inside of `zdapp`, meaning you will have two cget-envs at this point.

### Using a cget-env from Cmake directly ###

Using the cget command is fine, but if you're used to CMake you may already have a workflow that suits you. Here's how to integrate the cget-env's we've been creating above with a typical CMake workflow.

First, let's head back to the root, make a build directory, and configure CMake from within it:

    cd ..
    mkdir build
    cmake -DCMAKE_TOOLCHAIN_FILE=../cget/cget/cget.cmake -H../zdapp -B./

We can then build from this directory like normal:

     cmake --build ./

Or by invoking the native build system such as Visual Studio.

### Using alternative toolchains ###

But what if you already use toolchain files?

Here's an example of how to tell cget to use the toolchain file for Emscripten:

    mkdir embuild
    cget init --toolchain "${EMSCRIPTEN}/cmake/Modules/Platform/Emscripten.cmake"
